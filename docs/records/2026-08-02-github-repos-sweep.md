...
- 公開リポジトリの per-repo 併置(§12.3)は **5/5 で充足**(skills を本スイープで追加。§12.2 の一覧を更新済み)
  —— (→ 同日の追記節2で **7/7** に。claude-code-sandbox-experiments と muti-repo-workspace を追加)
...
| リポジトリ | 実施内容 | 到達 |
|---|---|---|
| self-analysis | 未コミット8本を commit(`.gitignore` 新設)→ 容器外 A **9本**を `docs/records/` へ + README に文書表 | **master 到達済み**(`f8b3edc`) |
...
| muti-repo-workspace | 未コミット21本を commit(PR #15)→ 導入 + 容器外 A **22本**を `docs/records/` へ + `plan.md` §6 に参照文(PR #17) | **master 到達済み**(`2aebc90`) |

- **移行の完了条件は「移動」と「参照文」の両方**(§5.2)。リンク検証は「壊れていないこと」しか見ず、
  **移した先が誰からも参照されていない状態を検出しない**。mrw は移行22本の L 側被参照が0だったため、
  生きた進行管理文書 `plan.md` に §6(記録索引・役割つき)を置いて閉じた。co-working は README.md を
  新設し、self-analysis は README に文書表を足した。**4件とも参照文を持って閉じている**
...
保留4件は**全件が本流到達済み**(§5.3 の完了条件を満たす)。残る継続項目は保留ではなく sekisho の2件のみ:

- sekisho: `main` への合流(定期合流待ち)/ `design/16〜19` のクラスタ移行(正文15の凍結後)

**PR を積むときの落とし穴**(実測): mrw は管理変更を別 PR にするため #16 の base を #15 のブランチに
置いたが、**#15 を `--delete-branch` でマージした時点で GitHub が #16 を自動クローズした**
(base ブランチ消失。**クローズ済み PR は base の付け替えも再オープンもできない**)。#17 として
出し直して解決。積む場合は base ブランチを消さないか、下段のマージ後に上段の base を master へ
**先に**付け替える。
